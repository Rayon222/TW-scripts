(() => {
  ...
  const ROW_STATE_KEY = 'rayon_fake_planner_row_state_v1';
  ...
  panel.innerHTML = `
    <style>
      ...
      #${APP_ID} .rowStatus{width:42px;text-align:center;font-size:18px;cursor:pointer;user-select:none}
      #${APP_ID} tr.row-yellow td{background:#fff3b8}
      #${APP_ID} tr.row-green td{background:#ccefd5}
      #${APP_ID} tr.row-yellow:hover td{background:#ffed9a}
      #${APP_ID} tr.row-green:hover td{background:#b8e8c4}
    ...
    <div class="head" id="rfpHead">
      <div class="title">🎭 Rayon Fake Planner v2.15-dev</div>
    ...
    <div class="body" id="rfpBody">
      ...
      <section class="card" style="margin-top:14px">
        ...
        <div class="tableWrap"><table>
          <thead><tr><th>Stav</th><th>#</th><th>Zdroj</th><th>Body</th><th>Cíl</th><th>Vzd.</th><th>Armáda</th><th>Odeslat</th><th>Dopad</th><th>Výsledek</th><th>Akce</th></tr></thead>
  ...
  function loadRowStates() {
    try {
      const saved = JSON.parse(localStorage.getItem(ROW_STATE_KEY) || '{}');
  ...
  function saveRowStates() {
    localStorage.setItem(ROW_STATE_KEY, JSON.stringify(state.rowStates));
  }

  state.rowStates = loadRowStates();
  ...
  function renderPlan() {
    const icons = ['⚪', '🟡', '🟢'];
    const rowClasses = ['', 'row-yellow', 'row-green'];

    el('#rfpResults').innerHTML = state.plan.map((row, index) => {
      const manualState = Number(state.rowStates[index] || 0) % 3;
      return `
      <tr class="${rowClasses[manualState]}">
        <td class="rowStatus" data-row-index="${index}" title="Kliknutím změnit stav">${icons[manualState]}</td>
        <td>${index + 1}</td>
  ...
  el('#rfpResults').addEventListener('click', event => {
    const statusCell = event.target.closest('.rowStatus');
    if (!statusCell) return;
    const index = Number(statusCell.dataset.rowIndex);
    if (!Number.isInteger(index)) return;
    state.rowStates[index] = (Number(state.rowStates[index] || 0) + 1) % 3;
    saveRowStates();
    renderPlan();
  });
