(() => {
  ...
  const STORAGE_KEY = "rayonTrainCatcherCourtyardV5State";
  ...
  app.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;background:#c6a66b;border-bottom:1px solid #7d510f">
      <div>
        <b style="font-size:19px">Rayon – Chytání jednoho vlaku PRO v5 – Nádvoří</b>
        <div style="font-size:12px;margin-top:2px">Jedno okno: nádvoří, potvrzení i ruční zrušení pomocí ENTER</div>
      </div>
  ...
  const buildPlan = () => {
    ...
    try {
      ...
      $("#rtClock").style.background = "#111";
      saveState();
      status("Plán vytvořen. Načítám nádvoří a spouštím odpočet.");
      loadCourtyardInFrame();
      startCurrent();
    } catch (error) {
