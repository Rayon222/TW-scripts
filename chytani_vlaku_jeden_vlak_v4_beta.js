(() => {
  ...
  const STORAGE_KEY = "rayonTrainCatcherSingleV4State";
  ...
  app.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;background:#c6a66b;border-bottom:1px solid #7d510f">
      <div>
        <b style="font-size:19px">Rayon – Chytání jednoho vlaku PRO v4 BETA</b>
        <div style="font-size:12px;margin-top:2px">Jeden vybraný vlak, předvyplnění jednotek a opakované pokusy</div>
      </div>
    ...
    <div style="padding:14px">
      ...
      <div style="margin-top:12px;padding:10px;background:#fff7df;border:1px solid #9b6b22;border-radius:5px">
        <b>2. Odesílací vesnice a jednotky</b>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px;margin-top:8px">
          <label>
            ...
            <select id="rtVillageMode" style="width:100%;height:34px;margin-top:4px">
              <option value="same">Stejná vesnice pro všechny pokusy</option>
              <option value="rotate">Další pokus z další vesnice</option>
            </select>
          </label>
          <label>
            Jednotka
          ...
          <button id="rtLoadVillages" type="button" style="height:34px;align-self:end">NAČÍST VESNICE</button>
        </div>
        <label style="display:block;margin-top:9px">
          Vesnice pro pokusy
          <select id="rtVillageSelect" multiple size="5" style="width:100%;box-sizing:border-box;margin-top:4px;font-family:monospace"></select>
          <small>Ve stejném režimu označ jednu vesnici. V režimu „další vesnice“ označ více vesnic v požadovaném pořadí.</small>
        </label>
      </div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px">
        ...
        <button id="rtPrepare" type="button" style="padding:8px 14px;font-weight:bold">PŘIPRAVIT ODESLÁNÍ</button>
  ...
  const villageForAttempt = (attemptNumber) => {
    const ids = selectedVillageIds();
    if (!ids.length) return currentVillage()?.id || "";
    if ($("#rtVillageMode").value === "same") return ids[0];
    return ids[(attemptNumber - 1) % ids.length];
  };
  ...
  const prepareCurrentSend = () => {
    ...
    const villageId = item.villageId || villageForAttempt(item.attemptNumber || 1);
    const unit = item.unit || $("#rtUnit").value;
    const count = item.unitCount || Math.max(1, Number($("#rtUnitCount").value) || 1);

    const popup = openAndPrefill(villageId, train.coord, unit, count);
    if (!popup) {
      status("Okno se nepodařilo otevřít. Povol v prohlížeči vyskakovací okna.");
      return;
    }

    status(`Otevřena vesnice ${villageId}; skript se pokouší předvyplnit ${count}× ${unit}. Finální odeslání je ruční.`);
  };
  ...
  const restoreState = () => {
    try {
      ...
      $("#rtVillageMode").value = state.villageMode || "same";
  ...
  const buildPlan = () => {
    ...
    try {
      ...
      const unit = $("#rtUnit").value;
      const unitCount = Math.max(1, Number($("#rtUnitCount").value) || 1);
      ...
      if (!villageIds.length && !currentVillage()) return status("Nejdříve načti a vyber odesílací vesnici.");
      ...
      $("#rtClock").style.background = "#111";
      saveState();
      status(`Plán vytvořen pro jeden vlak. První pokus: vesnice ${villageForAttempt(1)}, ${unitCount}× ${unit}.`);
    } catch (error) {
  ...
  $("#rtHelp").onclick = () => {
    showModal({
      ...
      body: `
        ...
        <p><b>NAČÍST VESNICE</b><br>Načte tvoje vesnice. Pro stejnou vesnici vyber jednu, pro střídání označ více.</p>
        <p><b>VYTVOŘIT PLÁN</b><br>Vytvoří plán pouze pro jeden vybraný vlak.</p>
        <p><b>PŘIPRAVIT ODESLÁNÍ</b><br>Otevře nádvoří správné vesnice, nastaví cíl a pokusí se předvyplnit zvolenou jednotku a počet. Odeslání musíš potvrdit ručně.</p>
        <p><b>SPUSTIT NEJBLIŽŠÍ AKCI</b><br>Spustí odpočet k nejbližšímu odeslání nebo zrušení.</p>
        ...
        <p><b>BETA:</b> Tribal Wars může mít na různých světech odlišné HTML formuláře. Když se jednotka nepředvyplní, pošli screenshot nádvoří a upravíme selektor.</p>
      `,
  ...
  $("#rtPrepare").onclick = prepareCurrentSend;
