(async () => {
  try {
    ...
    const SETTINGS_KEY = 'mk_map_marker_v5_settings';
    ...
    if (window.__mkMapMarkerV4?.destroy) {
      try { window.__mkMapMarkerV4.destroy(); } catch (_) {}
    }
    ...
    const monitor = () => {
      ...
      // Důležité: překresluj i tehdy, když TW po znovuotevření mapy
      // ponechá stejný handler nebo použije jiný kontejner mapy.
      if (hasVillagesInDom) repaintAll();
    };
