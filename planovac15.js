        ...
        for (let t of targetData) {
          const candidates = src
            ...
            .map((v) => {
              ...
              return { ...v, target: t.coord, dist, travel, send, adjustedLand, shifted, url };
            })
            .sort(
              (a, b) =>
                a.dist - b.dist ||
          ...
          rows
            .map(
              (r) => {
                ...
                return `[*][url=${sourceUrl}][coord]${r.coord}[/coord][/url][|][url=${targetMapUrl}][coord]${r.target}[/coord][/url][|]${r.dist.toFixed(2)}[|]${D(r.travel)}[|]${FD(r.send)}[|]${FT(r.send)}[|]${FD(r.adjustedLand)}[|]${FT(r.adjustedLand)}[|][url=${r.url}][b]ODESLAT[/b][/url][/*]`;
              },
            )
            .join("\n") +
