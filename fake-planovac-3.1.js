...
<small>Skript se pokusí použít všech pět druhů a rozdělit populační limit co nejrovnoměrněji.</small>
    ...
    async function loadVillages() {
      try {
        ...
        // 2) Jednotky načteme ze samostatného přehledu jednotek a spojíme podle ID vesnice.
        ...
        for (const unitsUrl of unitsUrls) {
          try {
            ...
            unitsDoc.querySelectorAll("tr").forEach((tr) => {
              const info = rowVillage(tr);
              if (!info) return;
              ...
              for (const u of FAKE_UNITS) {
                ...
                for (const sel of selectors) {
                  const el = tr.querySelector(sel);
                  if (el) {
                    amount = parseNumber(el.textContent);
                    break;
                  }
                }
        ...
        $("#mkvillages").innerHTML = villages
          .map((v) => {
            ...
            const suffix =
              ` · ${v.points || "?"} b.` +
              (unitTotal ? ` · jednotky OK` : ` · jednotky 0`);
        ...
        const withUnits = villages.filter(
          (v) => Object.values(v.units).some((n) => n > 0),
        ).length;

        msg(
          `Skupina „${groupName}“: ${villages.length} vesnic · body ${withPoints}/${villages.length} · jednotky ${withUnits}/${villages.length}.` +
          (!unitsLoaded
            ? " Přehled jednotek se nepodařilo správně načíst."
            : ""),
        );
      } catch (e) {
        villages = [];
    ...
    const calculatePlan = async (copyAfter = false) => {
      ...
      const noTroops = src.filter(
        (v) => !Object.values(v.units || {}).some((n) => Number(n) > 0),
      );
      if (noTroops.length)
        return msg(`U ${noTroops.length} zdrojových vesnic se nepodařilo načíst jednotky. Zkus znovu načíst skupinu.`);
