(() => {
  ...
  const APP_ID = 'fake-commander-v1';
  const STORAGE_KEY = 'fake_commander_settings_v1';
  const POSITION_KEY = 'fake_commander_position_v1';
  ...
  panel.innerHTML = `
    ...
    <div class="head" id="rfpHead">
      <div class="title">⚔️ Fake Commander v1.1</div>
    ...
    <div class="body" id="rfpBody">
      <div class="grid">
        <section class="card">
          <h2>1. Skupiny a zdrojové vesnice</h2><div class="info">Vesnice se interně rozlišují pouze podle souřadnice (např. 550|442). Název vesnice se pro limit nikdy nepoužívá.</div>
          <div class="row">
          ...
          <button class="btn primary" id="rfpLoadGroups" style="margin-top:9px">NAČÍST VESNICE Z VYBRANÝCH SKUPIN</button>
        ...
        <section class="card">
          <h2>3. Čas dopadu a zákaz odesílání</h2>
          <div class="row">
          ...
          <div class="row">
            <div><label>Neodesílat od (čas odeslání)</label><input id="rfpNightFrom" type="time" value="00:00"></div>
            <div><label>Neodesílat do (čas odeslání)</label><input id="rfpNightTo" type="time" value="07:00"></div>
          </div>
          <div class="row">
          ...
          <div class="info warn">Plánuje pouze dopady v zadaném intervalu. Příkazy, jejichž čas odeslání spadne do zakázaného intervalu, automaticky vynechá.</div>
          ...
          <label>Max. počet útoků z jedné zdrojové vesnice</label>
          <select id="rfpPerVillage">
      ...
      <div class="actions">
        <button class="btn primary" id="rfpBuild">VYTVOŘIT PLÁN FAKŮ</button>
        <button class="btn" id="rfpCopy">KOPÍROVAT PLÁN</button>
  ...
  async function loadGroups() {
    ...
    for (const url of pages) {
      try {
        ...
        console.warn('Fake Commander: skupiny z této stránky nešly načíst', url, error);
      }
    }
  ...
  function parseVillageRows(doc) {
    ...
    for (const table of $$('table', doc)) {
      ...
      for (const row of $$('tr', table)) {
        ...
        // Vybereme skutečný odkaz na přehled dané vesnice, ne libovolný odkaz
        // obsahující parametr village= z jiného sloupce řádku.
        const villageLink =
          row.querySelector('span.quickedit-vn a[href*="village="]') ||
          row.querySelector('a[href*="screen=overview"][href*="village="]') ||
          [...row.querySelectorAll('a[href*="village="]')].find(link =>
            extractCoord(link.closest('td')?.textContent || '') === villageCoord
          );
        ...
        let villageId = '';
        try {
          villageId = new URL(villageLink.href, location.href).searchParams.get('village') || '';
        } catch {}

        // ID vesnice musí být čistě číselné. Hodnoty typu "j12341" jsou chybně
        // rozpoznané odkazy a nesmí se dostat do plánu.
    ...
    return uniqueBy(villages, village => village.coord);
  }
  ...
  async function loadSelectedGroups() {
    ...
    // Slučujeme podle souřadnice vesnice, ne podle názvu ani podle chybně
    // rozpoznaného ID. Souřadnice je skutečný unikátní kód vesnice ve světě.
    ...
    for (const village of allVillages) {
      ...
      if (!existing) {
        ...
        if (!(Number(existing.points) > 0) && Number(village.points) > 0) {
          existing.points = village.points;
        }

        // Ponecháme pouze platné číselné ID.
        if (!/^\d+$/.test(String(existing.id)) && /^\d+$/.test(String(village.id))) {
          existing.id = village.id;
        }
      }
    }
  ...
  async function buildPlan() {
    ...
    const perVillage = Math.min(5, Math.max(1, Math.floor(Number(el('#rfpPerVillage').value) || 1)));
    ...
    // Zůstatek i limit držíme podle souřadnice, tedy skutečného kódu vesnice.
    const remainingByVillage = new Map(state.villages.map(village => [village.coord, { ...(village.army || {}) }]));
    const jobs = [];
    ...
    for (const job of jobs) {
      const sourceKey = job.village.coord;
      const alreadyUsed = usedByCoord.get(sourceKey) || 0;

      // Tvrdý bezpečnostní limit: souřadnice vesnice nikdy nesmí být použita
      // vícekrát než dovoluje nastavení, bez ohledu na názvy nebo ID.
      if (alreadyUsed >= perVillage) continue;

      const remainingArmy = remainingByVillage.get(sourceKey) || {};
      ...
      usedByCoord.set(sourceKey, alreadyUsed + 1);
    ...
    // Finální kontrola hotového plánu.
    const validation = new Map();
    for (const row of state.plan.filter(item => item.send)) {
      const key = row.village.coord;
      validation.set(key, (validation.get(key) || 0) + 1);
    }
    const broken = [...validation.entries()].filter(([, count]) => count > perVillage);
    if (broken.length) {
      throw new Error(`Interní kontrola limitu selhala: ${broken.map(([coord, count]) => `${coord}=${count}`).join(', ')}`);
    }
  ...
  function renderPlan() {
    el('#rfpResults').innerHTML = state.plan.map((row, index) => `
      <tr>
        <td>${index + 1}</td>
        <td><b>${esc(row.village.coord)}</b><div class="small">ID ${esc(row.village.id)}</div></td>
  ...
  el('#rfpCsv').onclick = () => {
    ...
    link.download = 'fake_commander_plan.csv';
    link.click();
