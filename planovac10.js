(async () => {
  ...
  <small style="display:block;margin-top:7px">Příklad: datum 2026-07-17 a čas 20:00:00.000</small>
    ...
    const calculatePlan = async (copyAfter = false) => {
      ...
      msg(
        ignoreMax
          ? `Hotovo: ${rows.length} příkazů z ${src.length} ručně vybraných vesnic na ${targets.length} cílů.${blocked ? ` Vynecháno kvůli nastavenému času: ${blocked}.` : ""}`
          : `Hotovo: ${rows.length} příkazů, maximálně ${maxPerTarget} na každý z ${targets.length} cílů.${blocked ? ` Vynecháno kvůli nastavenému času: ${blocked}.` : ""}`,
      );
