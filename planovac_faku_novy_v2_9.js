(() => {
  ...
  panel.innerHTML = `
    ...
    <div class="head" id="rfpHead">
      <div class="title">🎭 Rayon Fake Planner v2.7 v2.6</div>
  ...
  function detectUnitColumns(table) {
    const columns = {};
    const headerRows = $$('thead tr, tr', table).slice(0, 4);
    for (const row of headerRows) {
      [...(row.cells || [])].forEach((cell, index) => {
        const unit = unitFromElement(cell);
        if (unit && columns[unit] == null) columns[unit] = index;
      });
      if (Object.keys(columns).length >= 4) break;
    }
    return columns;
  }
  ...
  function parseArmyRows(doc) {
    ...
    // Přehled jednotek může obsahovat více tabulek nebo více řádků pro stejnou
    // vesnici. Proto nikdy nepřepisujeme celou armádu posledním nalezeným řádkem;
    // jednotlivé jednotky bezpečně slučujeme podle nejvyšší nalezené hodnoty.
    const tables = $$('table', doc)
      .map(table => ({ table, columns: detectUnitColumns(table) }))
      .filter(item => Object.keys(item.columns).length >= 2)
      .sort((a, b) => Object.keys(b.columns).length - Object.keys(a.columns).length);

    for (const { table, columns } of tables) {
      for (const row of $$('tbody tr, tr', table)) {
        const villageCoord = extractCoord(row.textContent);
        if (!villageCoord || !row.cells?.length) continue;

        const parsed = {};

        // Nejspolehlivější zdroj: indexy určené podle ikon v hlavičce tabulky.
        for (const [unit, index] of Object.entries(columns)) {
          if (!KNOWN_UNITS.includes(unit) || index >= row.cells.length) continue;
          parsed[unit] = numberFromText(row.cells[index]?.textContent);
        }

        // Záloha: některé světy mají data-unit/class přímo na buňce.
        [...row.cells].forEach(cell => {
          const unit = unitFromElement(cell);
          if (!unit || !KNOWN_UNITS.includes(unit)) return;
          const value = numberFromText(cell.textContent);
          if (Number.isFinite(value)) parsed[unit] = Math.max(parsed[unit] || 0, value);
        });

        if (!Object.keys(parsed).length) continue;
        const merged = result.get(villageCoord) || {};
        for (const unit of KNOWN_UNITS) {
          const value = Number(parsed[unit]);
  ...
  function makeStrongComposition(remainingArmy) {
    ...
    // Více než 50 katapultů: použijeme katapulty (max. 100).
    // 50 nebo méně katapultů: katapulty nepoužijeme a nahradíme je berany (max. 100).
    const useCatapults = availableCatapults > 50;
    const composition = {
