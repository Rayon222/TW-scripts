(async () => {
  ...
  <small style="display:block;margin-top:7px">Příklad: datum 2026-07-17 a čas 20:00:00.000</small>
    ...
    const $ = (s) => ov.querySelector(s),
      ...
      villageLabel = (name, coord) => {
        const clean = String(name || "")
          .replace(new RegExp(`\\(?\\s*${coord.replace("|", "\\|")}\\s*\\)?`, "g"), "")
          .replace(/\s+/g, " ")
          .trim();
        return clean ? `${clean} (${coord})` : coord;
      },
      CS = (s) => {
        let out = [],
          seen = new Set();
      ...
      FCompact = (ms) => {
        let d = new Date(ms),
          z = (n) => String(n).padStart(2, "0");
        return `${z(d.getUTCDate())}.${z(d.getUTCMonth() + 1)}. ${FT(ms)}`;
      },
      D = (ms) => {
      ...
      nextAllowedSend = (ms, from, to) => {
        if (!blockedTime(ms, from, to)) return ms;
        const d = new Date(ms),
          v = d.getUTCHours() * 60 + d.getUTCMinutes(),
    ...
    try {
      const savedLanding = JSON.parse(localStorage.getItem("mk111planner_landing") || "null");
      if (savedLanding?.date) $("#mklanddate").value = savedLanding.date;
      if (savedLanding?.time) $("#mklandtime").value = savedLanding.time;
    } catch (e) {}
    ...
    const saveSettings = () => {
      localStorage.setItem("mk111planner_landing", JSON.stringify({
        date: $("#mklanddate").value,
        time: $("#mklandtime").value,
      }));
      const data = {
        unit: $("#mkunit").value,
        title: $("#mktitle").value,
      ...
      try {
        ...
        try {
          savedGroups = JSON.parse(
            localStorage.getItem("mk111planner_groups") || "[]",
          );
        } catch (e) {}
        if (!Array.isArray(savedGroups) || !savedGroups.length) {
          const oldSavedGroup = localStorage.getItem("mk111planner_group");
          if (oldSavedGroup) savedGroups = [oldSavedGroup];
        }

        const wantedGroups = savedGroups.filter((id) => groups.has(String(id)));
        const fallbackGroup =
          current && groups.has(current) ? [current] : ["0"];
        const groupsToSelect = wantedGroups.length ? wantedGroups : fallbackGroup;

        [...$("#mkgroup").options].forEach((option) => {
          option.selected = groupsToSelect.includes(String(option.value));
        });
    ...
    const selectedGroupIds = () => {
      const ids = [...$("#mkgroup").selectedOptions]
    ...
    const selectedGroupNames = () => {
      const names = [...$("#mkgroup").selectedOptions]
        .map((o) => o.textContent.replace(/\s+/g, " ").trim())
        .filter(Boolean);
      return names.length ? names : ["Všechny vesnice"];
    };

    const updateVillageCount = () => {
      const total = villages.length;
      const selected = $("#mkvillages").selectedOptions.length;
      const used = selected || total;
      $("#mkvillagecount").textContent =
        selected > 0
          ? `Vybráno: ${selected} z ${total}. Do plánu se použije ${used}.`
          : `Vybráno: 0 z ${total}. Bez označení se použije všech ${total}.`;
      $("#mkvillageslabel").textContent =
        `Vesnice ve vybrané skupině (${used})`;
    };
    ...
    async function loadVillages() {
      try {
        ...
        msg(
          groupIds.length === 1
            ? `Načítám skupinu „${groupNames[0]}“…`
            : `Načítám ${groupIds.length} skupiny/skupin…`,
        );
        ...
        for (let i = 0; i < groupIds.length; i++) {
          ...
          const url = `/game.php?screen=overview_villages&mode=combined&group=${encodeURIComponent(gid)}&page=-1`;
        ...
        msg(
          villages.length
            ? `${groupIds.length === 1 ? `Skupina „${groupNames[0]}“` : `Vybrané skupiny (${groupNames.join(", ")})`}: ${villages.length} unikátních vesnic.`
            : `Ve vybraných skupinách nebyly nalezeny vesnice.`,
        );
      } catch (e) {
        villages = [];
    ...
    async function targetId(coord) {
      try {
        ...
        const walkName=(o)=>{
          if(!o||typeof o!=="object") return;
          if(!name && (o.name||o.label||o.text)) name=o.name||o.label||o.text;
          Object.values(o).forEach(v=>{if(typeof v==="object") walkName(v);});
        };
        try{walkName(j);}catch(e){}
    ...
    $("#mkvillages").onchange = updateVillageCount;
    ...
    const calculatePlan = async (copyAfter = false) => {
      ...
      for (let tg of targets) {
        let coord = `${tg[0]}|${tg[1]}`,
          tinfo = await targetId(coord);
        targetData.push({ tg, coord, tid: tinfo?.id || tinfo, targetName: tinfo?.name || coord });
      }
      let rows = [],
        blocked = 0,
        usedSources = new Set(),
          ...
          const candidates = src
            ...
            .map((v) => {
              const a = C(v.coord),
                ...
                originalSend = land - travel,
                send = blockNight
                  ? nextAllowedSend(originalSend, blockFrom, blockTo)
                  : originalSend,
                adjustedLand = send + travel,
                shifted = send !== originalSend,
                url = `${base}?village=${v.id}&screen=place&x=${t.tg[0]}&y=${t.tg[1]}${t.tid ? `&target=${t.tid}` : ""}`;
              return { ...v, target: t.coord, targetName: t.targetName, dist, travel, send, adjustedLand, shifted, url };
            })
            .sort(
              (a, b) =>
                a.dist - b.dist ||
      ...
      let title = $("#mktitle").value.trim() || "Útok",
        ...
        targetSummary = targetData
          ...
          .join(" "),
        bb =
          `[b]${title}[/b]\n[b]Požadovaný dopad:[/b] ${F(land)}\n[b]Jednotka:[/b] [unit]${u.id}[/unit]\n[b]Skupina:[/b] ${group}\n\n[table]\n[**]Výchozí vesnice[||]Cíl[||]Odeslání[||]Dopad[||]ODESLAT[/**]\n` +
          rows
            .map(
              (r) => {
                const sourceUrl = `${base}?village=${r.id}&screen=overview`;
                const targetParts = C(r.target);
                const targetMapUrl = `${base}?screen=map&x=${targetParts[0]}&y=${targetParts[1]}`;
                return `[*][url=${sourceUrl}]${villageLabel(r.name, r.coord)}[/url][|][url=${targetMapUrl}]${villageLabel(r.targetName, r.target)}[/url][|]${FCompact(r.send)}[|]${FCompact(r.adjustedLand)}[|][url=${r.url}][b]ODESLAT[/b][/url][/*]`;
              },
            )
            .join("\n") +
      ...
      msg(
        ignoreMax
          ? `Hotovo: ${rows.length} příkazů. Každá z použitých vesnic je v plánu jen jednou.${src.length - usedSources.size ? ` Nevyužito: ${src.length - usedSources.size} vesnic.` : ""}${blocked ? ` Posunuto kvůli nočnímu zákazu: ${blocked}.` : ""}`
          : `Hotovo: ${rows.length} příkazů, ${effectivePerTarget} útoků na cíl${reducedFrom ? ` (sníženo z ${reducedFrom})` : ""}. Každá vesnice je použita maximálně jednou.${blocked ? ` Posunuto na pozdější dopad kvůli nočnímu zákazu: ${blocked}.` : ""}`,
      );
    ...
    $("#mklanddate").addEventListener("change", saveSettings);
    $("#mklandtime").addEventListener("change", saveSettings);
