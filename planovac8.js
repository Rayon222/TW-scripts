(async () => {
  ...
  <small style="display:block;margin-top:7px">Příklad: datum 2026-07-17 a čas 20:00:00.000</small>
    ...
    const calculatePlan = async (copyAfter = false) => {
      ...
      $("#mkout").value = bb;
      msg(
        `Hotovo: ${rows.length} příkazů z ${src.length} vesnic na ${targets.length} cílů.${blocked ? ` Vynecháno kvůli nastavenému času: ${blocked}.` : ""}`,
      );
    ...
    ["mkunit", "mktitle", "mkblocknight", "mkblockfrom", "mkblockto"].forEach(
      (id) => {
