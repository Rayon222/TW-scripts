...
function bbcode(){
  ...
  const groups=state.settings.groups.map(id=>state.groups.find(g=>g.id===id)?.name||id).join(", ");
  const unitLabel=state.settings.fakeType==="auto1pct"
    ?"Automaticky: max. 1 % bodů (sekery, lehká, špeh, beranidlo, katapult)"
  ...
  const head=[
    ...
    `[b]Jednotky:[/b] ${unitLabel}`,
    `[b]Režim výběru:[/b] ${state.settings.distribution}`,
    `[b]Skupina:[/b] ${groups||"-"}`,
    ``
  ];

  const rows=state.plan.map(r=>{
    ...
    const units=Object.entries(r.composition).map(([u,a])=>`${u}:${a}`).join(" ");
    const link=`[url=${commandUrl(r)}][b]ODESLAT[/b][/url]`;
    return `${source} | ${target} | ${dist} | ${duration} | ${send} | ${units} | ${link}`;
  });
...
<div class="card half"><h3>7. Vytvoření plánu</h3><div class="toolbar"><button id="fcBuild" class="primary">Vytvořit plán faků</button><button id="fcCopy">Kopírovat BBCode plán</button><button id="fcCsv">Stáhnout CSV</button><button id="fcClear">Vymazat plán</button></div><div id="fcStatus" class="status">Připraveno</div><div id="fcSummary" class="status">Útoky: 0</div></div>
