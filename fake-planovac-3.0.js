...
<small>Skript se pokusí použít všech pět druhů a rozdělit populační limit co nejrovnoměrněji.</small>
      ...
      try {
        ...
        doc.querySelectorAll("tr").forEach((tr) => {
          ...
          if (id && co && !map.has(id)) {
            const unitCount = (u) => {
              const el =
                tr.querySelector(`[data-unit="${u}"]`) ||
                tr.querySelector(`td.unit-item-${u}`) ||
                tr.querySelector(`td[class*="${u}"]`);
              const m = String(el?.textContent || "").replace(/[.\s]/g, "").match(/\d+/);
              return m ? Number(m[0]) : 0;
            };

            let points = 0;
            const pEl =
              tr.querySelector('td[class*="points"]') ||
              tr.querySelector('[data-field="points"]') ||
              tr.querySelector('span[class*="points"]');
            if (pEl) {
              const pm = String(pEl.textContent || "").replace(/[.\s]/g, "").match(/\d+/);
              points = pm ? Number(pm[0]) : 0;
            }
            if (!points) {
              const nums = (String(tr.textContent || "").match(/\b\d{3,6}\b/g) || [])
                .map(Number)
                .filter((n) => n >= 100 && n <= 200000 && String(n) !== String(id));
            ...
            map.set(id, {
              ...
              name: (a.textContent || "").replace(/\s+/g, " ").trim(),
              points,
              units: Object.fromEntries(FAKE_UNITS.map((u) => [u, unitCount(u)])),
            });
          }
        });
        villages = [...map.values()];
        $("#mkvillages").innerHTML = villages
          .map(
            (v) =>
              `<option value="${esc(v.id)}">${esc(v.coord + (v.name && !v.name.includes(v.coord) ? " – " + v.name : "") + (v.points ? " · " + v.points + " b." : " · body ?"))}</option>`,
          )
          .join("");
    ...
    const calculatePlan = async (copyAfter = false) => {
      ...
      if (src.some((v) => !v.points))
        return msg("U některých zdrojových vesnic se nepodařilo načíst body. Otevři kombinovaný přehled vesnic s body a jednotkami a spusť skript znovu.");
