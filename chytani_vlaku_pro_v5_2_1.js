(() => {
  ...
  const APP_ID = "rayon-train-catcher-pro-v521";
  const STORAGE_KEY = "rayonTrainCatcherV521State";
  ...
  app.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;background:#c6a66b;border-bottom:1px solid #7d510f">
      <div>
        <b style="font-size:19px">Rayon – Chytání jednoho vlaku PRO v5.2.1</b>
  ...
  const showRuntimeError = (error) => {
    ...
    const statusBox = $("#rtStatus");
    if (statusBox) {
      statusBox.style.color = "#9b0000";
      statusBox.textContent = `Chyba skriptu: ${message}`;
    }
    console.error("[Rayon v5.2.1]", error);
  };

  window.addEventListener("error", (event) => {
    if (document.getElementById(APP_ID)) showRuntimeError(event.error || event.message);
  });
  ...
  const bind = (selector, eventName, handler) => {
    ...
    if (!element) {
      console.warn(`[Rayon v5.2.1] Chybí prvek ${selector}`);
      return;
    }
  ...
  bind("#rtModeLive", "onchange", () => { applyModeUi(); saveState(); });
  bind("#rtModeTest", "onchange", () => { applyModeUi(); saveState(); });
  bind("#rtBackSettings", "onclick", showSetupScreen);
  bind("#rtCancelPlan", "onclick", cancelPlan);
  bind("#rtSentNowHunt", "onclick", recordSentNow);
  bind("#rtDoneHunt", "onclick", markDoneAndNext);

  bind("#rtParseTrains", "onclick", parseTrainInput);
  bind("#rtLoadVillages", "onclick", loadVillages);
  bind("#rtBuild", "onclick", buildPlan);
  bind("#rtPrepare", "onclick", loadCourtyardInFrame);
  bind("#rtSentNow", "onclick", recordSentNow);
  bind("#rtDone", "onclick", markDoneAndNext);
  bind("#rtSkip", "onclick", skipCurrent);
  bind("#rtSave", "onclick", saveState);

  try {
    applyModeUi();
    restoreState();
    showSetupScreen();
    status("Skript v5.2.1 je připraven.");
  } catch (error) {
    showRuntimeError(error);
  }
})();