(async () => {
  ...
  <small style="display:block;margin-top:7px">Příklad: datum 2026-07-17 a čas 20:00:00.000</small>
    ...
    async function loadGroups() {
      try {
        ...
        $("#mkgroup").innerHTML = sorted
          ...
          .join("");
        if (current && groups.has(current)) $("#mkgroup").value = current;
    ...
    $("#mkcalc").onclick = async () => {
      ...
      msg(`Vytvářím plán pro ${src.length} vesnic a ${targets.length} cílů…`);
      let targetData = [];
      ...
      for (let v of src) {
        ...
        for (let t of targetData) {
          let dist = Math.hypot(a[0] - t.tg[0], a[1] - t.tg[1]),
            travel = dist * u.sp * 60000,
            send = land - travel,
            url = t.tid
              ? `${base}?village=${v.id}&screen=place&target=${t.tid}`
              : `${base}?village=${v.id}&screen=place&x=${t.tg[0]}&y=${t.tg[1]}`;
          if (blockNight && blockedTime(send, blockFrom, blockTo)) {
            blocked++;
            continue;
          }
          rows.push({ ...v, target: t.coord, dist, travel, send, url });
        }
      }
      rows.sort(
        (a, b) =>
      ...
      $("#mkout").value = bb;
      msg(
        `Hotovo: ${rows.length} příkazů.${blocked ? ` Vynecháno kvůli nastavenému času: ${blocked}.` : ""}`,
      );
    };
    $("#mkcopy").onclick = async () => {
      if (!$("#mkout").value) return msg("Nejdříve vypočítej plán.");
      try {
