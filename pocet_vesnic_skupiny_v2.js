(async () => {
  try {
    ...
    const loadGroups = async () => {
      ...
      const wantedNames = ["plný útok", "1/2 off", "malá off"];
      ...
      return [...groups]
        .filter(([id, name]) => wantedNames.includes(normalized(name)))
        .sort((a, b) => {
          const ai = wantedNames.indexOf(normalized(a[1]));
          const bi = wantedNames.indexOf(normalized(b[1]));
          return ai - bi;
        });
    };
    ...
    const run = async () => {
      try {
        ...
        if (!groups.length) throw new Error("Nebyly nalezeny skupiny Plný útok, 1/2 off ani Malá off.");
