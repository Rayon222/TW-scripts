(async () => {
  try {
    const APP_ID = 'mk_map_marker_v9_1';
    const STORAGE_KEY = 'mk_map_marker_v1_data';
    const SETTINGS_KEY = 'mk_map_marker_v9_1_settings';

    // Always replace an already running v9 build so CSS changes are applied immediately.
    for (const old of [
      window.__mkMapMarkerV9_1,
    ...
    const COLORS = [
      ...
      { key: 'off_train', label: 'OFF + VLAK', color: '#ff8c00' },
    ];
    ...
    const STYLE_ID = 'mk_map_marker_v9_1_style';
    ...
    markerStyle.textContent = `
      ...
      [data-mk-marker-color="green"] {
        outline: 2px solid #00a000 !important;
        outline-offset: -3px !important;
        z-index: 20 !important;
      }
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <b style="font-size:16px">Označení vesnic v9.1</b>
    ...
    const updateCounts = () => {
      ...
      panel.querySelectorAll('[data-color]').forEach((btn) => {
        ...
        btn.style.color = active && !['green', 'off_train'].includes(btn.dataset.color) ? '#fff' : '';
    ...
    const exportTargetsCsv = () => {
      ...
      const rows = [
        ...
        ...entries.map(([coord, mark]) => [
          ...
          coord,
          priority[mark.key] || '',
          mark.key === 'off_train' ? 'OFF_TRAIN' : 'NORMAL',
          1,
          ''
        ])
      ];
    ...
    const hookMap = () => {
      ...
      if (handler === hookedHandler && handler.onClick?.__mkMarkerV91) {
        repaintAll();
        return true;
      }
      ...
      const wrappedOnClick = function (x, y, event) {
        ...
        if (marks[coord]?.key === selected.key) {
          ...
          marks[coord] = { key: selected.key, color: selected.color, shape: 'circle' };
      ...
      wrappedOnClick.__mkMarkerV91 = true;
    ...
    const monitor = () => {
      ...
      if (handler) {
        if (handler !== hookedHandler || !handler.onClick?.__mkMarkerV911) {
          hookMap();
        } else {
          ensureObserver();
        }
    ...
    const destroy = () => {
      ...
      markerStyle.remove();
      panel.remove();
      delete window.__mkMapMarkerV9_1;
    };
    ...
    hookMap();
    repaintAll();
    msg('Aktivní. Všechna označení jsou kruhová; označení přes Shift + klik.');
  } catch (error) {
    console.error('Map Marker v9.1', error);
