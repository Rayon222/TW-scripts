(async () => {
  try {
    const APP_ID = 'mk_map_marker_v7';
    const STORAGE_KEY = 'mk_map_marker_v1_data';
    const SETTINGS_KEY = 'mk_map_marker_v7_settings';

    if (window.__mkMapMarkerV7?.showPanel) {
      window.__mkMapMarkerV7.showPanel();
      return;
    }
    ...
    const STYLE_ID = 'mk_map_marker_v7_style';
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <b style="font-size:16px">Označení vesnic v7</b>
    ...
    const hookMap = () => {
      ...
      if (handler === hookedHandler && handler.onClick?.__mkMarkerV7) {
        repaintAll();
        return true;
      }
      ...
      wrappedOnClick.__mkMarkerV7 = true;
    ...
    console.error('Map Marker v7', error);
