(() => {
  ...
  panel.innerHTML = `
    ...
    <div class="head" id="rfpHead">
      <div class="title">🎭 Rayon Fake Planner v2.14</div>
  ...
  function renderPlan() {
    ...
    el('#rfpResults').innerHTML = visibleRows.map(row => {
      ...
      <tr class="${rowClass}">
        ...
        <td>${row.send ? `
          <a class="btn good rfpOpenSend" target="_blank" href="${esc(buildSendUrl(row))}" data-row-index="${index}">ODESLAT</a>
          <button class="btn rfpMarkSent" data-row-index="${index}">${sendStatus === 'sent' ? 'Vrátit' : 'Potvrdit odeslání'}</button>` : '-'}</td>
      </tr>`;
    }).join('');
  ...
  el('#rfpResults').addEventListener('click', event => {
    const openLink = event.target.closest('.rfpOpenSend');
    if (openLink) {
      const row = state.plan[Number(openLink.dataset.rowIndex)];
      if (row) setSendStatus(row, 'opened');
      return;
    }
    ...
    if (markButton) {
      ...
      if (!row) return;
      setSendStatus(row, getSendStatus(row) === 'sent' ? 'new' : 'sent');
    }
  });
