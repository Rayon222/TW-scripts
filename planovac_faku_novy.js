(() => {
  ...
  function makeGreenComposition(village, remainingArmy) {
    ...
    if (!requiredPopulation) return { composition: null, reason: 'Vesnice nemá správně rozpoznané body' };

    const allowed = ['spy', 'ram', 'light', 'catapult'];
    const units = allowed.map(unit => ({
      unit,
      pop: Math.max(1, Number(state.unitInfo[unit]?.pop || 1)),
      available: Math.max(0, Number(remainingArmy[unit] || 0))
    })).filter(item => item.available > 0);

    if (!units.length) return { composition: null, reason: 'Chybí povolené jednotky pro zelený fake' };

    // Najde nejmenší možnou sestavu: nejdřív minimální přebytek populace,
    // potom nejmenší počet jednotek. Limit je přesně 1 % bodů vesnice.
    const maxUnitPop = Math.max(...units.map(item => item.pop));
    const searchLimit = requiredPopulation + maxUnitPop - 1;
    const dp = Array(searchLimit + 1).fill(null);
    dp[0] = { count: 0, composition: {} };

    for (const item of units) {
      // Pro výpočet stačí nejvýše tolik kusů, kolik se vejde do malého
      // hledaného rozsahu. Původní verze procházela všechny tisíce dostupných
      // jednotek a mohla zablokovat prohlížeč.
      const usefulCopies = Math.min(item.available, Math.floor(searchLimit / item.pop));
      for (let copy = 0; copy < usefulCopies; copy++) {
        for (let pop = searchLimit - item.pop; pop >= 0; pop--) {
          const current = dp[pop];
          if (!current) continue;
          const nextPop = pop + item.pop;
          const nextCount = current.count + 1;
          if (!dp[nextPop] || nextCount < dp[nextPop].count) {
            dp[nextPop] = {
              count: nextCount,
              composition: { ...current.composition, [item.unit]: (current.composition[item.unit] || 0) + 1 }
            };
          }
        }
      }
    ...
    let best = null;
    for (let pop = requiredPopulation; pop <= searchLimit; pop++) {
      if (!dp[pop]) continue;
      const candidate = { pop, ...dp[pop] };
      if (!best || candidate.pop < best.pop || (candidate.pop === best.pop && candidate.count < best.count)) best = candidate;
    }

    if (!best) return { composition: null, reason: `Nesplní zelený limit ${requiredPopulation} populace` };

    const composition = Object.fromEntries(allowed.map(unit => [unit, best.composition[unit] || 0]));
  ...
  function buildBBCode() {
    ...
    for (const row of successfulRows) {
      ...
      const sourceName = row.village.name || row.village.coord;
      const travelDuration = formatDuration(row.arrival - row.send);
      lines.push(
        `[*][b]${sourceName}[/b] [coord]${row.village.coord}[/coord]` +
