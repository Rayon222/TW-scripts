(() => {
  ...
  function detectPointsColumn(table) {
    const headerRows = $$('thead tr, tr', table).slice(0, 3);
    for (const row of headerRows) {
      for (let index = 0; index < row.cells.length; index++) {
        const cell = row.cells[index];
        const text = String(cell.textContent || '').trim().toLowerCase();
        const field = String(cell.dataset.field || cell.className || '').toLowerCase();
        if (/^(body|bodů|bodu|points?)$/.test(text) || /(^|\s)(points?|body)(\s|$)/.test(field)) return index;
      }
    }
    return -1;
  }
  ...
  function parsePoints(row, pointsColumn = -1) {
    const direct = row.dataset.points || row.getAttribute('data-points');
    if (direct) return numberFromText(direct);

    const explicit = row.querySelector('.points, td.points, [data-field="points"], [data-field="body"]');
    if (explicit) return numberFromText(explicit.textContent);

    if (pointsColumn >= 0 && row.cells?.[pointsColumn]) {
      const value = numberFromText(row.cells[pointsColumn].textContent);
      if (value > 0) return value;
    }

    // Bezpečný fallback: body vesnice jsou běžně 26–13000. Nebereme největší číslo
    // z celého řádku, protože to mohou být jednotky, suroviny nebo kapacita skladu.
    const candidates = [...row.cells]
      .map(cell => numberFromText(cell.textContent))
      .filter(value => value >= 26 && value <= 13000);
  ...
  function makeGreenComposition(village, remainingArmy) {
    ...
    // Herní minimum pro útočný příkaz je 100 populace. Zelený fake proto musí
    // splnit vyšší z hodnot: 1 % bodů cílové vesnice, nebo 100 populace.
    const requiredPopulation = Math.max(100, Math.ceil(villagePoints * 0.01));

    // Zelený fake se skládá pouze ze seker, lehké jízdy, beranidel a katapultů.
    // Potřebná populace (1 % bodů vesnice) se rozdělí co nejrovnoměrněji
    // mezi všechny typy, které jsou v dané vesnici skutečně dostupné.
    ...
    // První průchod: přibližně stejný podíl populace pro každý dostupný typ.
    for (let index = 0; index < units.length; index++) {
      const item = units[index];
      const typesLeft = units.length - index;
      const populationLeft = Math.max(0, requiredPopulation - currentPopulation);
      const share = populationLeft / typesLeft;
      const amount = Math.min(item.available, Math.floor(share / item.pop));
      item.used = amount;
      currentPopulation += amount * item.pop;
    }

    // Druhý průchod: doplní chybějící populaci. Přednost dostane typ,
    // který zatím přispěl nejmenším dílem; při posledním kroku se volí
    // jednotka s nejmenším možným překročením 1% hranice.
    while (currentPopulation < requiredPopulation) {
      const gap = requiredPopulation - currentPopulation;
      const candidates = units.filter(item => item.used < item.available);
      if (!candidates.length) break;

      const fitting = candidates.filter(item => item.pop <= gap);
      const pool = fitting.length ? fitting : candidates;
      pool.sort((a, b) => {
        const shareA = a.used * a.pop;
        const shareB = b.used * b.pop;
        if (shareA !== shareB) return shareA - shareB;
        if (!fitting.length && a.pop !== b.pop) return a.pop - b.pop;
        return allowed.indexOf(a.unit) - allowed.indexOf(b.unit);
      });

      const pick = pool[0];
      pick.used++;
      currentPopulation += pick.pop;
    }

    if (currentPopulation < requiredPopulation) {
      return { composition: null, reason: `Nesplní zelený limit ${requiredPopulation} populace` };
    }

    const composition = Object.fromEntries(allowed.map(unit => {
      const item = units.find(entry => entry.unit === unit);
      return [unit, item?.used || 0];
    }));
