(() => {
  ...
  async function loadWorldVillagePoints() {
    ...
    const text = await getText('/map/village.txt');
    const byId = new Map();
  ...
  async function loadGroup(groupId) {
    ...
    for (const village of villages) {
      ...
      const exactPoints = worldPoints?.byId.get(String(village.id)) ?? worldPoints?.byCoord.get(village.coord);
      if (Number.isFinite(exactPoints) && exactPoints > 0) village.points = exactPoints;
    }
    return villages;
  }
