(() => {
  ...
  function buildBBCode() {
    return state.plan.filter(row => row.send).map((row, index) =>
      `${index + 1}. [coord]${row.village.coord}[/coord] → [coord]${row.target}[/coord] | ${compositionText(row.composition)} | ${row.distance.toFixed(1)} polí | odeslat [b]${formatDateTime(row.send)}[/b] | dopad ${formatDateTime(row.arrival)}`
    ).join('\n');
  }
