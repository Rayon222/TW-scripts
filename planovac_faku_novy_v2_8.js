  ...
  function parseArmyRows(doc) {
    ...
    for (const table of $$('table', doc)) {
      ...
      for (const row of $$('tr', table)) {
        ...
        // Primárně čteme podle rozpoznaných hlaviček sloupců.
        for (const [unit, index] of Object.entries(columns)) {
          army[unit] = numberFromText(row.cells[index]?.textContent);
        }

        // Záloha pro světy/tabulky, kde hlavička katapultu nebo beranu není
        // standardní: rozpoznáme jednotku přímo z atributů každé buňky.
        [...row.cells].forEach(cell => {
          const unit = unitFromElement(cell);
          if (!unit || !KNOWN_UNITS.includes(unit)) return;
          const value = numberFromText(cell.textContent);
          if (value >= 0) army[unit] = value;
        });
