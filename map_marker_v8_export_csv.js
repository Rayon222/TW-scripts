  ...
  try {
    ...
    const exportTargetsCsv = () => {
      const entries = Object.entries(marks)
        ...
        .sort((a, b) => {
          const pa = { black: 1, green: 2, pink: 3 }[a[1].key] || 99;
          const pb = { black: 1, green: 2, pink: 3 }[b[1].key] || 99;
      ...
      const colorLabel = { black: 'Černá', green: 'Zelená', pink: 'Pink' };
      const priority = { black: 1, green: 2, pink: 3 };
      const csvCell = (value) => `"${String(value ?? '').replace(/"/g, '""')}"`;
      const rows = [
        ['Barva', 'Souřadnice', 'Priorita', 'Počet útoků', 'Poznámka'],
    ...
    const hookMap = () => {
      ...
      wrappedOnClick.__mkMarkerV8 = true;
