      ...
      let title = $("#mktitle").value.trim() || "Útok",
        ...
        bb =
          ...
          rows
            .map(
              (r) =>
                `[*][coord]${r.coord}[/coord][|][coord]${r.target}[/coord][|]${r.dist.toFixed(2)}[|]${D(r.travel)}[|]${FD(r.send)}[|]${FT(r.send)}[|]${FD(r.adjustedLand)}[|]${FT(r.adjustedLand)}[|][url=${r.url}][b]ODESLAT[/b][/url][/*]`,
            )
            .join("\n") +
