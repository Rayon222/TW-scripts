(() => {
  ...
  const parseDkLine = (line) => {
    ...
    const commands = [...text.matchAll(/\[command\]([^\[]+)\[\/command\]/gi)]
      .map((m) => m[1].trim());
    const coordMatch = text.match(/\[coord\](\d{1,3}\|\d{1,3})\[\/coord\]/i);
    const playerMatch = text.match(/\[player\]([^\[]+)\[\/player\]/i);
    ...
    let year = Number(arrivalMatch[3]);
    if (year < 100) year += 2000;

    const timestamp = new Date(
      year,
      Number(arrivalMatch[2]) - 1,
      Number(arrivalMatch[1]),
      Number(arrivalMatch[4]),
      Number(arrivalMatch[5]),
      Number(arrivalMatch[6]),
      Number(arrivalMatch[7].padEnd(3, "0"))
    ).getTime();
    ...
    return {
      ...
      attackType: commands.find((c) => /^attack_/i.test(c)) || "attack",
      unit: commands.find((c) => !/^attack_/i.test(c)) || "",
    };
  };

  const attackLabel = (type) => {
    const labels = {
      attack_small: "malý útok",
      attack_medium: "střední útok",
      attack_large: "velký útok",
    };
    return labels[type] || type || "útok";
  };
  ...
  panel.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:#c6a66b;border-bottom:1px solid #7d510f">
      <strong style="font-size:19px">Chytání vlaku – import z DK</strong>
    ...
    <div style="padding:14px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <label>
          ...
          <textarea id="mkTcArrivals" rows="10" placeholder="[command]attack_large[/command] [command]snob[/command] [coord]567|449[/coord] --> Čas příchodu: 17.07.26 21:53:07:773 [player]Rayon222[/player]" style="width:100%;box-sizing:border-box;margin-top:4px"></textarea>
          <small>Vlož přímo zkopírovaný výpis z DK. Skript načte typ útoku, jednotku, souřadnice, čas příchodu a hráče.</small>
        </label>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin:14px 0">
        <button id="mkTcNow" type="button">Přidat aktuální serverový čas</button>
  ...
  $("#mkTcCalc").addEventListener("click", () => {
    ...
    if (!lines.length) {
      message("Vlož alespoň jeden řádek z DK.");
      return;
    }
    ...
    for (const line of lines) {
      const dk = parseDkLine(line);
      if (dk) {
        arrivals.push(dk);
        continue;
      }

      const plain = parseDateTime(line);
      if (plain !== null) {
        arrivals.push({
          raw: line,
          time: plain,
    ...
    if (invalid.length) {
      message(
        `Nepodařilo se načíst ${invalid.length} řádků. Zkontroluj text „Čas příchodu“.`
      );
      return;
    }
    ...
    arrivals.forEach((arrival, arrivalIndex) => {
      ...
      const details = [
        attackLabel(arrival.attackType),
        arrival.unit,
        arrival.coord,
        arrival.player,
      ].filter(Boolean).join(" | ");

      output.push(`ÚTOK ${arrivalIndex + 1}: ${details || "bez dalších údajů"}`);
      output.push(`Dopad: ${formatDateTime(arrival.time)}`);
    ...
    const coords = new Set(arrivals.map((a) => a.coord).filter(Boolean));
    message(
      `Hotovo: načteno ${arrivals.length} útoků z DK` +
      (coords.size ? ` na ${coords.size} vesnic` : "") +
      `, maximálně ${optionCount} variant na každý.`
    );
  });
