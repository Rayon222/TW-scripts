(() => {
  ...
  app.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;background:#c6a66b;border-bottom:1px solid #7d510f">
      <div>
        <b style="font-size:19px">Rayon – Chytání vlaků PRO v3</b>
    ...
    <div style="padding:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:10px;margin-top:11px">
        <label>
          Limit zrušení
        ...
        <label>
          ...
          <small>mezi prvními pokusy.</small>
        </label>
        <label>
          Další pokus za
  ...
  const renderQueue = () => {
    ...
    queue.forEach((item, index) => {
      ...
      lines.push(
        ...
        `vlak ${item.trainIndex + 1} / pokus ${item.attemptNumber || 1}${item.coord ? ` ${item.coord}` : ""}  ${formatTime(item.time)}` +
  ...
  const renderCurrent = () => {
    ...
    const label = item.mode === "send" ? "ODESLAT" : "ZRUŠIT";
    $("#rtAction").textContent = `${label} – vlak ${item.trainIndex + 1}, pokus ${item.attemptNumber || 1}${item.coord ? ` – ${item.coord}` : ""}`;
  ...
  $("#rtHelp").onclick = () => {
    showModal({
      ...
      body: `
        <p><b>VYTVOŘIT PLÁN</b><br>Načte útoky z DK, vypočítá časy a sestaví společnou frontu.</p>
        ...
        <hr>
        <p><b>Další pokusy:</b> Po odpovědi „NE – DALŠÍ POKUS“ se vytvoří nový pokus pro stejný vlak. Opakovat lze tolikrát, kolikrát dovolí zbývající čas před cílovým návratem.</p>
      `,
