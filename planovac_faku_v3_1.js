...
try{
 const p=JSON.parse(localStorage.getItem("rfp_v3_1_pos")||"null");
...
document.addEventListener("mouseup",()=>{
 if(!drag)return;
 localStorage.setItem("rfp_v3_1_pos",JSON.stringify({left:panel.style.left,top:panel.style.top}));
 drag=null;
});
...
function validGroupName(name){
 ...
 return n && !/^(skupiny?|groups?|vybrat skupinu|select group)$/i.test(n);
}

function addGroup(out,id,name){
 id=String(id??"").trim();
 name=String(name||"").replace(/\s+/g," ").trim();
 if(!/^\d+$/.test(id)||!validGroupName(name)) return;
 if(!out.some(g=>g.id===id)) out.push({id,name});
}

function parseGroupOptions(doc){
 const out=[];

 // Starší i nový výběr skupin v horní liště přehledu.
 for(const s of $$('select#group_select,select[name="group"],select[name="group_id"]',doc)){
  for(const o of [...s.options]) addGroup(out,o.value,o.textContent);
 }

 // Novější světy zobrazují skupiny jako odkazy / položky popup menu.
 for(const a of $$('a[href*="group="],a[data-group-id],[data-group-id]',doc)){
  let id=a.getAttribute('data-group-id')||'';
  if(!id && a.getAttribute('href')){
   try{id=new URL(a.getAttribute('href'),location.href).searchParams.get('group')||'';}catch{}
  }
  addGroup(out,id,a.textContent||a.getAttribute('title'));
 }

 // Záloha pro skupiny vložené do JavaScriptu jako JSON.
 for(const sc of $$('script',doc)){
  const text=sc.textContent||'';
  const re=/(?:"|')?(?:group_id|id)(?:"|')?\s*:\s*(?:"|')?(\d+)(?:"|')?[^{}]{0,160}?(?:"|')?(?:name|label)(?:"|')?\s*:\s*(?:"|')([^"'\n]+)(?:"|')/gi;
  let m;
  while((m=re.exec(text))) addGroup(out,m[1],m[2]);
 }
 return out;
}
...
async function initGroups(){
 const collected=[];
 const merge=list=>list.forEach(g=>addGroup(collected,g.id,g.name));
 merge(parseGroupOptions(document));

 const village=encodeURIComponent(gd.village?.id||"");
 const urls=[
  `/game.php?village=${village}&screen=overview_villages&mode=combined`,
  `/game.php?village=${village}&screen=overview_villages&mode=units`,
  `/game.php?village=${village}&screen=groups`
 ];
 ...
 for(const url of urls){
  ...
  try{
   ...
   merge(parseGroupOptions(new DOMParser().parseFromString(html,"text/html")));
  }catch(e){console.debug("Načtení skupin selhalo:",url,e);}
 }

 groups=collected;
 if(!groups.some(g=>g.id==="0")) groups.unshift({id:"0",name:"Všechny vesnice"});
 renderGroups();
}
...
el("#rfpCsv").onclick=()=>{
 ...
 a.download="rayon_fake_plan_v3.csv";
 a.click();
