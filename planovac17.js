          ...
          rows
            .map(
              (r) => {
                ...
                return `[*][url=${sourceUrl}]${villageLabel(r.name, r.coord)}[/url][|][url=${targetMapUrl}]${villageLabel(r.targetName, r.target)}[/url][|]${D(r.travel)}[|]${FD(r.send)}[|]${FT(r.send)}[|]${FD(r.adjustedLand)}[|]${FT(r.adjustedLand)}[|][url=${r.url}][b]ODESLAT[/b][/url][/*]`;
              },
            )
            .join("\n") +
