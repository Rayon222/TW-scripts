(async () => {
  try {
    const APP_ID = 'mk_map_marker_v6';
    const STORAGE_KEY = 'mk_map_marker_v1_data';
    const SETTINGS_KEY = 'mk_map_marker_v6_settings';

    if (window.__mkMapMarkerV6?.showPanel) {
      window.__mkMapMarkerV6.showPanel();
      return;
    }

    for (const old of [window.__mkMapMarkerV4, window.__mkMapMarkerV5]) {
      if (old?.destroy) {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <b style="font-size:16px">Označení vesnic v6</b>
    ...
    const hookMap = () => {
      ...
      if (handler === hookedHandler && handler.onClick?.__mkMarkerV6) {
        repaintAll();
        return true;
      }
      ...
      wrappedOnClick.__mkMarkerV6 = true;
    ...
    const startRepaintBurst = () => {
      clearInterval(repaintBurstTimer);

      const started = Date.now();
      const run = () => {
        const handler = window.TWMap?.map?.handler;
        if (handler && (handler !== hookedHandler || !handler.onClick?.__mkMarkerV6)) {
          hookMap();
        }
        repaintAll();
        ...
        if (Date.now() - started >= 10000) {
          clearInterval(repaintBurstTimer);
          repaintBurstTimer = null;
        }
      };

      run();
      repaintBurstTimer = window.setInterval(run, 200);
    };

    const monitor = () => {
      const handler = window.TWMap?.map?.handler;
      const hasVillagesInDom = !!document.querySelector('[id^="map_village_"]');
      ...
      if (hasVillagesInDom && !mapWasVisible) {
        startRepaintBurst();
      } else if (hasVillagesInDom && !repaintBurstTimer) {
        repaintAll();
      }

      mapWasVisible = hasVillagesInDom;
    };
    ...
    pageObserver = new MutationObserver((mutations) => {
      const mapChanged = mutations.some((mutation) =>
        Array.from(mutation.addedNodes || []).some((node) =>
          node.nodeType === 1 &&
          (node.matches?.('[id^="map_village_"], #map_wrap, #map, #map_container, .map_container, .map-wrapper') ||
           node.querySelector?.('[id^="map_village_"]'))
        )
      );
      if (mapChanged) {
        setTimeout(() => {
    ...
    monitorTimer = window.setInterval(monitor, 200);
    ...
    window.addEventListener('focus', () => {
      monitor();
      if (document.querySelector('[id^="map_village_"]')) startRepaintBurst();
    });
    ...
    console.error('Map Marker v6', error);
