(() => {
  ...
  panel.innerHTML = `
    <style>
      ...
      #${APP_ID} tr.send-opened,#${APP_ID} tr.send-opened td{background:#ffe680!important}
      #${APP_ID} tr.send-sent,#${APP_ID} tr.send-sent td{background:#b9edc4!important}
    ...
    <div class="head" id="rfpHead">
      <div class="title">🎭 Rayon Fake Planner v2.16</div>
  ...
  function renderPlan() {
    ...
    el('#rfpResults').innerHTML = visibleRows.map(row => {
      ...
      return `
      <tr class="${rowClass}" data-send-id="${esc(rowSendId(row))}">
        <td>${index + 1}</td>
        ...
        <td>${row.send ? (
          sendStatus === 'new'
            ? `<button class="btn good rfpOpenSend" type="button" data-send-id="${esc(rowSendId(row))}">ODESLAT</button>`
            : sendStatus === 'opened'
              ? `<button class="btn good rfpMarkSent" type="button" data-send-id="${esc(rowSendId(row))}">✅ POTVRDIT</button>
                 <button class="btn rfpResetOne" type="button" data-send-id="${esc(rowSendId(row))}">↩ VRÁTIT</button>`
              : `<button class="btn rfpResetOne" type="button" data-send-id="${esc(rowSendId(row))}">↩ VRÁTIT</button>`
        ) : '-'}</td>
      </tr>`;
    }).join('');

    // Přímé handlery na každém tlačítku. Nepoužíváme delegované kliknutí ani href,
    // aby do události nemohla zasáhnout hra nebo jiný skript.
    const rowBySendId = sendId => state.plan.find(item => item.send && rowSendId(item) === sendId);

    $$('.rfpOpenSend', el('#rfpResults')).forEach(button => {
      button.onclick = event => {
        event.preventDefault();
        event.stopPropagation();
        const row = rowBySendId(button.dataset.sendId);
        if (!row) {
          setStatus('Řádek útoku nebyl nalezen.', true);
          return;
        }

        // Nejprve uložit stav. Poté okamžitě vizuálně změnit konkrétní řádek.
        state.sendStatuses[rowSendId(row)] = 'opened';
        saveSendStatuses();
        const tableRow = button.closest('tr');
        tableRow?.classList.add('send-opened');
        const statusCell = tableRow?.querySelector('td:nth-last-child(2)');
        if (statusCell) {
          statusCell.className = 'status-opened';
          statusCell.textContent = '🟨 Otevřeno';
        }
        button.textContent = '🟨 OTEVŘENO';
        button.disabled = true;

        // Otevření proběhne až po uložení a vizuální změně.
        const url = buildSendUrl(row);
        const sendWindow = window.open(url, '_blank');
        if (!sendWindow) {
          button.disabled = false;
          button.textContent = 'ODESLAT';
          setStatus('Prohlížeč zablokoval nové okno. Povol vyskakovací okna pro Tribal Wars.', true);
          return;
        }

        // Kompletní překreslení až s malým zpožděním; stav už je uložený.
        setTimeout(renderPlan, 150);
      };
    });

    $$('.rfpMarkSent', el('#rfpResults')).forEach(button => {
      button.onclick = event => {
        event.preventDefault();
        event.stopPropagation();
        const row = rowBySendId(button.dataset.sendId);
    ...
    $$('.rfpResetOne', el('#rfpResults')).forEach(button => {
      button.onclick = event => {
        event.preventDefault();
        event.stopPropagation();
        const row = rowBySendId(button.dataset.sendId);
