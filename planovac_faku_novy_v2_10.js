(() => {
  ...
  function detectUnitColumns(table) {
    ...
    // Hledáme ikony jednotek ve všech hlavičkových řádcích před prvním
    // řádkem vesnice. Na některých světech nejsou ikony v prvních 4 řádcích.
    for (const row of $$('tr', table)) {
      if (extractCoord(row.textContent)) break;
      [...(row.cells || [])].forEach((cell, index) => {
        const candidates = [cell, ...$$('img,[data-unit],[class*="unit_"]', cell)];
        for (const candidate of candidates) {
          const unit = unitFromElement(candidate);
          if (unit && columns[unit] == null) {
            columns[unit] = index;
            break;
          }
        }
  ...
  function parseArmyRows(doc) {
    ...
    for (const table of $$('table', doc)) {
      ...
      for (const row of $$('tr', table)) {
        ...
        // Primární metoda: přesný index sloupce podle ikony v hlavičce.
        ...
        // Záloha: data-unit nebo unit_* přímo na buňce/jejím obsahu.
