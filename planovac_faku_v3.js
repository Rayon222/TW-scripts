...
const APP_ID = "rayon-fake-planner-v3";
...
<div class="head" id="rfpHead">
 <div class="title">🎭 Rayon Fake Planner v3</div>
...
try{
 const p=JSON.parse(localStorage.getItem("rfp_v3_pos")||"null");
...
document.addEventListener("mouseup",()=>{
 if(!drag)return;
 localStorage.setItem("rfp_v3_pos",JSON.stringify({left:panel.style.left,top:panel.style.top}));
 drag=null;
});
...
function parseGroupOptions(doc){
 const s=doc.querySelector("#group_select,select[name='group']");
 if(!s) return [];
 return [...s.options].map(o=>({id:o.value,name:o.textContent.trim()}));
}

async function initGroups(){
 groups=parseGroupOptions(document);
 if(!groups.length){
  const html=await getText(`/game.php?village=${encodeURIComponent(gd.village?.id||"")}&screen=overview_villages`);
  groups=parseGroupOptions(new DOMParser().parseFromString(html,"text/html"));
 }
 groups=groups.filter((g,i,a)=>g.name&&a.findIndex(x=>x.id===g.id)===i);
 if(!groups.length) groups=[{id:"0",name:"Všechny vesnice"}];
 renderGroups();
}
