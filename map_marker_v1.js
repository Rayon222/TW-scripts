  ...
  try {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <b style="font-size:16px">Označení vesnic</b>
    ...
    const originalSpawnSector = TWMap.map.handler.spawnSector;
    ...
    const handleSpawnSector = function (...args) {
      const result = originalSpawnSector.apply(this, args);
      setTimeout(repaintAll, 0);
      return result;
    };

    TWMap.map.handler.onClick = handleMapClick;
    TWMap.map.handler.spawnSector = handleSpawnSector;

    const destroy = () => {
      TWMap.map.handler.onClick = originalOnClick;
      TWMap.map.handler.spawnSector = originalSpawnSector;
      clearVisible();
      panel.remove();
