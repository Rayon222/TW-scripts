(() => {
  ...
  const APP_ID = "rayon-train-catcher-v531";
  const STORAGE_KEY = "rayonTrainCatcherV531";
  ...
  app.innerHTML = `
    <style>
      ...
      #${APP_ID} .unit-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}
      #${APP_ID} .unit{display:grid;grid-template-columns:minmax(112px,1fr) 76px;gap:8px;align-items:center;
        min-height:38px;background:#f8edcf;padding:6px 8px;border-radius:5px;border:1px solid #dec48b}
      #${APP_ID} .unit-name{font-size:14px;font-weight:bold;white-space:nowrap}
      #${APP_ID} .unit input[type=number]{width:76px;height:30px;min-width:76px;padding:4px 6px;
        font-size:15px;font-weight:bold;text-align:center;border:1px solid #8b672d;border-radius:4px;background:#fff}
      #${APP_ID} .main-action{width:100%;height:58px;font-size:22px!important;margin-top:8px}
      #${APP_ID} .time-panel{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}
      #${APP_ID} .time-box{background:#171717;color:#fff;border-radius:6px;padding:7px;text-align:center}
      #${APP_ID} .time-label{font-size:11px;letter-spacing:.7px;color:#e9cf93;margin-bottom:3px}
      #${APP_ID} .time-value{font:700 25px monospace;white-space:nowrap}
      #${APP_ID} .countdown{font:700 25px monospace}
      ...
      @media(max-width:680px){
        #${APP_ID}{width:94vw}
        #${APP_ID} .rt-grid{grid-template-columns:1fr!important}
        #${APP_ID} .unit-grid{grid-template-columns:1fr!important}
        #${APP_ID} .time-panel{grid-template-columns:1fr!important}
      }
    </style>
    <div class="rt-head" id="rtDrag">
      <div class="rt-title">Rayon Train Catcher v5.3.1 · <span id="rtVillage"></span></div>
      <div style="text-align:right">
        <div style="font-size:10px;opacity:.85">SERVEROVÝ ČAS</div>
    ...
    <div class="rt-body">
      ...
      <div class="time-panel">
        <div class="time-box">
          <div class="time-label">ODPOČET / STOPKY</div>
          <div id="rtCountdown" class="time-value countdown">PŘIPRAVENO</div>
        </div>
        <div class="time-box">
          <div class="time-label">PŘESNÝ SERVEROVÝ ČAS AKCE</div>
          <div id="rtActionTime" class="time-value">--:--:--.---</div>
        </div>
      </div>
  ...
  const loadUnitInfo = async () => {
    ...
    $("#rtUnits").innerHTML = units.map((u) => `
      <label class="unit">
        <span class="unit-name">${esc(UNIT_LABELS[u.id] || u.id)}</span>
        <input id="rtCount_${u.id}" type="number" min="0" step="1" value="${u.id === "spy" ? 5 : 0}" data-unit="${u.id}" aria-label="Počet ${esc(UNIT_LABELS[u.id] || u.id)}">
      </label>
    `).join("");
  ...
  const recordSent = () => {
    ...
    $("#rtActionTime").textContent = formatClock(active.cancelAt);
    startCountdown();
