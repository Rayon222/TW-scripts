(async () => {
  try {
    const APP_ID = 'mk_map_marker_v5';
    const STORAGE_KEY = 'mk_map_marker_v1_data';
    const SETTINGS_KEY = 'mk_map_marker_v4_settings';

    if (window.__mkMapMarkerV5?.showPanel) {
      window.__mkMapMarkerV5.showPanel();
      return;
    }

    // Při načtení nové verze odpoj starou verzi, aby nevznikly dvojité handlery.
    window.__mkMapMarkerV4?.destroy?.();
    ...
    const paint = (x, y) => {
      ...
      el.style.borderRadius = mark?.shape === 'circle' ? '50%' : '0';
    };
    ...
    panel.innerHTML = `
      ...
      <div id="mkmm_body" style="margin-top:8px">
        ...
        <label style="display:flex;align-items:center;gap:7px;margin-top:9px;padding:6px 8px;background:#fff7df;border:1px solid #9b6b22;border-radius:5px;cursor:pointer">
          <input id="mkmm_circle" type="checkbox">
          <b>Kruh</b>
        </label>
    ...
    $('#mkmm_circle').addEventListener('change', () => {
      ...
      saveSettings();
      msg(settings.circle ? 'Nová označení budou kruhy.' : 'Nová označení budou čtverce.');
    });
    ...
    const hookMap = () => {
      ...
      if (handler === hookedHandler && handler.onClick?.__mkMarkerV5) {
        repaintAll();
        return true;
      }
      ...
      const wrappedOnClick = function (x, y, event) {
        ...
        const selectedShape = settings.circle ? 'circle' : 'square';
        if (marks[coord]?.key === selected.key && (marks[coord]?.shape || 'square') === selectedShape) {
          delete marks[coord];
          msg(`${coord}: označení odstraněno.`);
        } else {
          marks[coord] = { key: selected.key, color: selected.color, shape: selectedShape };
          msg(`${coord}: ${selected.label}, ${selectedShape === 'circle' ? 'kruh' : 'čtverec'}.`);
        }
        saveMarks();
        paint(x, y);
        updateCounts();
      ...
      wrappedOnClick.__mkMarkerV5 = true;
    ...
    console.error('Map Marker v5', error);
