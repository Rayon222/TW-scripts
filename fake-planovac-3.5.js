...
<small>Pokud není dost vhodných zdrojových vesnic, plán vytvoří jen dostupné útoky a zobrazí upozornění.</small>
      ...
      for (let t of targetData) {
        const candidates = src
          ...
          .filter(
            (candidate) =>
              candidate &&
              (sourceUsage.get(candidate.coord) || 0) < MAX_PER_SOURCE,
          )
          .sort(
            (a, b) =>
              a.dist - b.dist ||
        ...
        for (const candidate of candidates) {
          ...
          sourceUsage.set(
            candidate.coord,
            (sourceUsage.get(candidate.coord) || 0) + 1,
          );
          addedForTarget++;
        }

        const requestedForTarget = ignoreMax ? src.length : maxPerTarget;
        if (addedForTarget < requestedForTarget) {
          shortages.push({
            target: t.coord,
            planned: addedForTarget,
            requested: requestedForTarget,
          });
        }
      }
      rows.sort(
