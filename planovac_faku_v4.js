...
const APP_ID = "rayon-fake-planner-v4";
...
<div class="head" id="rfpHead">
 <div class="title">🎭 Rayon Fake Planner v4</div>
...
try{
 const p=JSON.parse(localStorage.getItem("rfp_v4_pos")||"null");
...
document.addEventListener("mouseup",()=>{
 if(!drag)return;
 localStorage.setItem("rfp_v4_pos",JSON.stringify({left:panel.style.left,top:panel.style.top}));
 drag=null;
});
...
async function initGroups(){
 ...
 for(const url of urls){
  ...
  }catch(e){console.warn("Skupiny:",url,e);}
 }
 try{
  const r=await fetch("/game.php?screen=groups&ajax=load_group_menu",{credentials:"same-origin",cache:"no-store",headers:{"TribalWars-Ajax":"1"}});
  ...
  try{
   ...
   const walk=o=>{
    ...
    if(id!==undefined&&/^\d+$/.test(String(id))&&validGroupName(name)) map.set(String(id),String(name).trim());
    Object.values(o).forEach(walk);
   };
   walk(data);
  }catch(e){addGroupsFromDoc(new DOMParser().parseFromString(txt,"text/html"),map);}
 }catch(e){console.warn("Skupiny AJAX:",e);}
 groups=[...map.entries()].map(([id,name])=>({id,name})).filter(g=>g.id==="0"||validGroupName(g.name)).sort((a,b)=>a.id==="0"?-1:b.id==="0"?1:a.name.localeCompare(b.name,"cs",{sensitivity:"base"}));
 renderGroups();
...
function parseArmyRows(doc){
 const map=new Map();
 const supported=["spear","sword","axe","archer","spy","light","marcher","heavy","ram","catapult"];
 for(const table of $$('table',doc)){
  ...
  headers.forEach((th,i)=>{
   ...
   const text=(th.getAttribute('data-unit')||img?.src||th.className||"");
   const m=String(text).match(/unit_(spear|sword|axe|archer|spy|light|marcher|heavy|ram|catapult)|\b(spear|sword|axe|archer|spy|light|marcher|heavy|ram|catapult)\b/);
   const u=m?.[1]||m?.[2]; if(u) cols[u]=i;
  });
  for(const row of table.querySelectorAll('tbody tr, tr')){
   const c=coord(row.textContent); if(!c) continue;
   const cells=[...row.cells], army={};
   for(const u of supported){
    const direct=row.querySelector(`[data-unit="${u}"],td.unit-item-${u},td[class~="${u}"]`);
    if(direct) army[u]=num(direct.textContent);
    else if(cols[u]!==undefined&&cells[cols[u]]) army[u]=num(cells[cols[u]].textContent);
   }
   if(Object.values(army).some(n=>n>0)) map.set(c,army);
  }
 }
 return map;
}
async function loadOneGroup(groupId){
 const common=`/game.php?screen=overview_villages&group=${encodeURIComponent(groupId)}&page=-1`;
 const [combined,units]=await Promise.all([
  getText(common+"&mode=combined"),
  getText(common+"&mode=units&type=home")
 ]);
...
el("#rfpCsv").onclick=()=>{
 ...
 a.download="rayon_fake_plan_v4.csv";
 a.click();
 setTimeout(()=>URL.revokeObjectURL(a.href),1000);
};

function saveFakeSettings(){
 const data={targets:el("#rfpTargets").value,from:el("#rfpFrom").value,to:el("#rfpTo").value,nightFrom:el("#rfpNightFrom").value,nightTo:el("#rfpNightTo").value,minDistance:el("#rfpMinDistance").value,spacing:el("#rfpSpacing").value,perVillage:el("#rfpPerVillage").value,mode:panel.querySelector('input[name="rfpMode"]:checked')?.value||"limit"};
 localStorage.setItem("rfp_v4_settings",JSON.stringify(data));
}
function loadFakeSettings(){
 try{
  const d=JSON.parse(localStorage.getItem("rfp_v4_settings")||"{}");
  for(const [id,key] of [["#rfpTargets","targets"],["#rfpFrom","from"],["#rfpTo","to"],["#rfpNightFrom","nightFrom"],["#rfpNightTo","nightTo"],["#rfpMinDistance","minDistance"],["#rfpSpacing","spacing"],["#rfpPerVillage","perVillage"]]) if(d[key]!=null) el(id).value=d[key];
  const radio=panel.querySelector(`input[name="rfpMode"][value="${d.mode}"]`); if(radio) radio.checked=true;
 }catch(e){}
}
["#rfpTargets","#rfpFrom","#rfpTo","#rfpNightFrom","#rfpNightTo","#rfpMinDistance","#rfpSpacing","#rfpPerVillage"].forEach(id=>el(id).addEventListener("change",saveFakeSettings));
panel.querySelectorAll('input[name="rfpMode"]').forEach(x=>x.addEventListener("change",saveFakeSettings));

(async()=>{
 try{loadFakeSettings();await loadUnitInfo();await initGroups();setStatus("Připraveno. Vyber skupiny a načti je.");}
