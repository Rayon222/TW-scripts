          ...
          rows
            .map(
              (r) => {
                ...
                return `[*][url=${sourceUrl}]${r.name || r.coord} (${r.coord})[/url][|][url=${targetMapUrl}]${r.targetName || ""} (${r.target})[/url][|]${r.dist.toFixed(2)}[|]${D(r.travel)}[|]${FD(r.send)}[|]${FT(r.send)}[|]${FD(r.adjustedLand)}[|]${FT(r.adjustedLand)}[|][url=${r.url}][b]ODESLAT[/b][/url][/*]`;
              },
            )
            .join("\n") +
